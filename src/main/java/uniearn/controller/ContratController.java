...
public class ContratController {
    ...
    private void openContratDialog(Contrat contrat) {
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible d'ouvrir le dialogue: " + ex.getMessage());
        }
    }
    ...
    private void editContrat() {
        ...
        if (selected != null) {
            ...
            showAlert(Alert.AlertType.WARNING, "Sélection requise", "Veuillez sélectionner un contrat à modifier");
        }
    }
    ...
    private void deleteContrat() {
        Contrat selected = tableContracts.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showAlert(Alert.AlertType.WARNING, "Sélection requise", "Veuillez sélectionner un contrat à supprimer");
            return;
        }
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            if (contratService.deleteContrat(selected.getIdContract())) {
                ...
                showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible de supprimer le contrat");
            }
        }
    }
    ...
    private void signByClient() {
        Contrat selected = tableContracts.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showAlert(Alert.AlertType.WARNING, "Sélection requise", "Veuillez sélectionner un contrat");
            return;
        }

        if (contratService.signByClient(selected.getIdContract())) {
            showAlert(Alert.AlertType.INFORMATION, "Succès", "Contrat signé par le client");
            loadContrats();
        } else {
            showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible de signer le contrat");
        }
    }
    ...
    private void signByFreelancer() {
        ...
        if (contratService.signByFreelancer(selected.getIdContract())) {
            showAlert(Alert.AlertType.INFORMATION, "Succès", "Contrat signé par le freelancer");
            loadContrats();
        } else {
            showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible de signer le contrat");
        }
