...
public class AdminPaymentsController {
    ...
    public void loadPayments() {
        new Thread(() -> {
            try {
                System.out.println("🔍 DIAGNOSTIC: Loading payments from 'payment_escrow'...");
                List<PaymentEscrow> payments = escrowService.getAllEscrows();
                
                for (PaymentEscrow p : payments) {
                    System.out.println("   💰 Record: ID=" + p.getId() + 
                                       ", Contract=" + p.getContractId() + 
                                       ", Status='" + p.getStatus() + "' (Raw)");
                }
    ...
    private void updateStatistics(List<PaymentEscrow> payments) {
        ...
        try {
            List<PaymentEscrow> allPayments = escrowService.getAllEscrows();
    ...
    private void handleSync() {
        try {
            int count = escrowService.syncMissingEscrows();
            if (count > 0) {
                showAlert("✅ Synchronisation réussie", count + " contrats ont été synchronisés et ajoutés à la liste.");
            } else {
                showAlert("ℹ️ Info", "Toutes les données sont déjà synchronisées.");
            }
            loadPayments();
        } catch (Exception e) {
            showAlert("❌ Erreur", "Échec de la synchronisation: " + e.getMessage());
        }
    }
}
