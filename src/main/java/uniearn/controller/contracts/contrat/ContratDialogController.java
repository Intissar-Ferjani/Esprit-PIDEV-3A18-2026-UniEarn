...
public class ContratDialogController {
    ...
    private void mapFieldsToContrat() {
        ...
        // title and content for DB compatibility
        if (contrat.getTitle() == null || contrat.getTitle().isEmpty()) {
            contrat.setTitle("Contrat Admin");
        }
        if (contrat.getContent() == null || contrat.getContent().isEmpty()) {
            contrat.setContent("Contrat créé par l'administrateur");
        }

        // projectID/paymentID don't exist in shared DB, keep in model only
