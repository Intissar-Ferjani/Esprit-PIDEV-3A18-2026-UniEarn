...
public class FreelancerContractController {
    ...
    private int currentFreelancerID = -1; // Default to -1 to avoid loading wrong data
    ...
    private void loadContracts() {
        System.out.println("DEBUG: FreelancerContractController.loadContracts() with freelancerID=" + currentFreelancerID);
        if (currentFreelancerID <= 0) {
            System.out.println("DEBUG: Skipping loadContracts because ID is invalid (" + currentFreelancerID + ")");
        ...
        try {
            ...
            System.out.println("DEBUG: Loaded " + (allContracts != null ? allContracts.size() : 0) + " contracts for freelancer " + currentFreelancerID);
    ...
    private void viewContract(Contrat contract) {
        try {
            // Fetch full contract data including images before showing details
            ...
            controller.setContratService(contratService);
            controller.setUserType("FREELANCER"); // Must be set BEFORE setContract()
            ...
            showError("Erreur", "Impossible d'ouvrir les détails du contrat: " + e.getMessage());
        }
    }

    private void signContract(Contrat contract) {
        try {
            // Fetch full contract data including images before signing
            Contrat fullContract = contratService.getContratById(contract.getIdContract());
            if (fullContract != null) {
                contract = fullContract;
            }
            ...
            controller.setUserType("FREELANCER"); // Must be set BEFORE setContract()
            controller.setContract(contract);
            controller.setOnSignatureComplete(() -> {
                loadContracts();
            });
            ...
            showError("Erreur", "Impossible d'ouvrir la fenêtre de signature: " + e.getMessage());
        }
    }
    ...
    public void setCurrentFreelancerID(int freelancerID) {
        System.out.println("DEBUG: FreelancerContractController.setCurrentFreelancerID(" + freelancerID + ")");
