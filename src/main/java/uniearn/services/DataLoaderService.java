...
public class DataLoaderService {
    ...
    /**
     * Récupère l'idClient à partir de l'idUser
     */
    public int getClientIdByUserId(int idUser) {
        String sql = "SELECT idClient FROM client WHERE userID = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            System.err.println("❌ Erreur lors de la récupération de l'idClient: " + e.getMessage());
            e.printStackTrace();
        }
        return -1;
    }
    ...
    /**
     * Récupère le nom du freelancer par son idFreelancer (FK dans contract)
     */
    public String getFreelancerNameByFreelancerId(int idFreelancer) {
        String sql = "SELECT u.name FROM user u JOIN freelancer f ON u.idUser = f.idUser WHERE f.idFreelancer = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            System.err.println("Error getting freelancer name by idFreelancer=" + idFreelancer + ": " + e.getMessage());
        }
        return "Inconnu";
    }
}