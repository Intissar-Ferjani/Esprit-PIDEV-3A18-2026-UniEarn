...
public class SignatureImageService {
    ...
    /**
     * Convertir une image JavaFX en tableau de bytes
     * Utilise une sérialisation ARGB simple
    ...
    public static byte[] imageToPngBytes(Image image) {
        ...
        try {
            ...
            // Créer un buffer pour les pixels
            ByteArrayOutputStream baos = new ByteArrayOutputStream();

            // Écrire les dimensions (4 bytes)
            baos.write((width >> 8) & 0xFF);
            baos.write(width & 0xFF);
            baos.write((height >> 8) & 0xFF);
            baos.write(height & 0xFF);

            // Lire les pixels de l'image et les écrire
            ...
            System.out.println("DEBUG: Image convertie en bytes - Taille: " + imageBytes.length + " bytes");
            return imageBytes;
        } catch (IOException e) {
            System.err.println("Erreur lors de la conversion de l'image en bytes: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
     ...
     * @param imageBytes Le tableau de bytes contenant les pixels ARGB
     * @return L'image JavaFX
     */
    public static Image bytesToImage(byte[] imageBytes) {
        if (imageBytes == null || imageBytes.length < 4) {
            System.err.println("ERROR: imageBytes insuffisant (< 4 bytes)");
            return null;
        }
        ...
        try {
            ...
            System.out.println("DEBUG bytesToImage: width=" + width + ", height=" + height);

            if (width <= 0 || height <= 0) {
                System.err.println("ERROR: Dimensions invalides: " + width + "x" + height);
                return null;
            }
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    if (pixelIndex + 4 <= imageBytes.length) {
                        ...
                        System.err.println("ERROR: Pas assez de bytes pour pixel (" + x + "," + y + ")");
                        break;
                    }
                }
            }

            System.out.println("DEBUG: Image créée avec succès: " + width + "x" + height);
            return image;
        } catch (Exception e) {
